...
export default function NotificationSetupScreen() {
  ...
  const showEnable =
    webPushState !== 'subscribed' && webPushState !== 'unsupported' && webPushState !== 'checking';

  return (
    <Screen>
      <BrandHeader eyebrow="Notifications" />
          ...
          <MetalButton
            ...
            label={
              pushBusy
                ? 'Enabling…'
                : webPushState === 'denied'
                  ? 'Try again — allow notifications'
                  : webPushState === 'install-required'
                    ? 'Enable alerts (after Home Screen)'
                    : 'Enable alerts'
            }
            onPress={onEnable}
          />
